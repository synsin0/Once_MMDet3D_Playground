...
from projects.mmdet3d_plugin.datasets.waymo.waymo_let_metric import compute_waymo_let_metric
...
class CustomWaymoDataset(KittiDataset):
    ...
    def format_results(self,
        ...
        # ipdb.set_trace()
        # print(result_files)
        # np.save('debug_eval/waymo_eval_result_kitti_format',result_files)## turn into cam-coord, it sucks
        # exit(0)
        # open('zlt_output_kitti_format_debug.txt','w').write(str(result_files))  #we got absolutely right data
        # exit(0)  

        if 'waymo' in data_format:
            from projects.mmdet3d_plugin.datasets.waymo.kitti2waymo import KITTI2Waymo
            waymo_root = osp.join(
